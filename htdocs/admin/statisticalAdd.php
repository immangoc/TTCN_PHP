...
if (isset($_GET['session_idA'])) {
    ...
    if ($resultA) {
        ...
        $stmt_query = $db->link->prepare($sql_query); 
        ...
        $stmt_thongke = $db->link->prepare($sql_thongke); 
        ...
        if ($resultC->num_rows == 0) {
            ...
            $sql_insert = "INSERT INTO tbl_thongke (date_thongke, soluong, doanhthu, donhang) 
                           VALUES (?, ?, ?, ?)";
            $stmt_insert = $db->link->prepare($sql_insert); 
            ...
            $sql_update_thongke = "UPDATE tbl_thongke SET soluong = ?, doanhthu = ?, donhang = ? 
                                   WHERE date_thongke = ?";
            $stmt_update_thongke = $db->link->prepare($sql_update_thongke); 
