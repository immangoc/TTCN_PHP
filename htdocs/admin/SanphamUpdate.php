...
if (isset($_GET['sanpham_id']) && !empty($_GET['sanpham_id'])) {
    ...
    if ($get_sanpham && $get_sanpham->num_rows > 0) {
        ...
        error_log("Sản phẩm không tồn tại với ID: $sanpham_id");
        $error_message = "Sản phẩm không tồn tại.";
    }
} else {
    error_log("Không có sanpham_id trong URL");
    $error_message = "ID sản phẩm không hợp lệ.";
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit']) && $resul) {
    $update_product = $product->update_product($_POST, $_FILES, $sanpham_id);
    if ($update_product === true) {
        $_SESSION['success_message'] = "Sửa sản phẩm thành công!"; 
        header('Location: Sanphamlist.php');
        exit();
    } else {
        $error_message = $update_product;
    }
}
?>
...
<head>
    ...
    <style>
        ...
        .tile { background-color: #fff; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); padding: 2rem; }
        .title-h3 { color: #343a40; font-weight: 600; margin-bottom: 2rem; }
        ...
        .error-message { color: #dc3545; font-size: 0.875rem; margin-bottom: 1rem; }
            ...
            <div class="tile">
                ...
                <div class="tile-body">
                    <div class="admin-content-right">
                                ...
                                <div class="product-add-content">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="form-group col-md-6">
                                            <label class="control-label">Chọn Size sản phẩm</label>
                                            <div class="sanpham-size">
                                                <?php
                                                $sizes = ['S', 'M', 'L', 'XL', 'XXL'];
                                                $get_sizes = $product->get_size($sanpham_id);
                                                $selected_sizes = [];
                                                if ($get_sizes) {
                                                    while ($size_result = $get_sizes->fetch_assoc()) {
                                                        $selected_sizes[] = $size_result['sanpham_size'];
                                                    }
                                                }
                                        ...
                                        <div class="form-group col-md-3">
                                            ...
                                            <input class="form-control" value="<?php echo htmlspecialchars($resul['soluong']); ?>" required type="number" min="0" name="soluong" id="soluong">
                                        </div>
                                        <div class="form-group col-md-3">
                                            <label class="control-label" for="giamgia">Giảm giá (%)</label>
                                            <input class="form-control" value="<?php echo htmlspecialchars($resul['giamgia']); ?>" type="number" min="0" max="100" name="giamgia" id="giamgia">
                                        </div>
                                    </div>
                                    ...
                                    <div class="row">
                                        <div class="form-group col-md-3">
                                            ...
                                            <input class="form-control" value="<?php echo htmlspecialchars($resul['sanpham_gia']); ?>" required type="number" min="0" name="sanpham_gia" id="sanpham_gia">
                                        </div>
                                        <div class="form-group col-md-3">
                                    ...
                                    <div class="row">
                                        <div class="form-group col-md-3">
                                            <label class="control-label" for="sanpham_anh">Ảnh đại diện</label>
                                            <input class="form-control" type="file" name="sanpham_anh" id="sanpham_anh" accept="image/*">
                                        ...
                                        <div class="form-group col-md-3">
                                            <label class="control-label" for="sanpham_anhs">Ảnh sản phẩm mô tả</label>
                                            <input class="form-control" type="file" multiple name="sanpham_anhs[]" id="sanpham_anhs" accept="image/*">
                        ...
                        <?php else: ?>
                            <div class="error-message">Không thể chỉnh sửa vì sản phẩm không tồn tại.</div>
                            <a href="Sanphamlist.php" class="btn btn-cancel">
                                <i class="fas fa-times me-1"></i> Quay lại danh sách
                            </a>
                        <?php endif; ?>
                    </div>
                </div>
